...
// está caído, la app admin sigue funcionando normal — solo no llegan notificaciones en vivo
// (la lista igual se carga por REST al montar el componente).
export function connectNotifications({ onNotification } = {}) {
  const token = authService.getToken()
  const claims = token ? decodeJwt(token) : null
  if (!token || !claims?.tnd_id) return null
  ...
  client = new Client({
    ...
    reconnectDelay: 5000,
    onConnect: () => {
      client.subscribe(`/topic/admin/${claims.tnd_id}`, (message) => {
        try {
